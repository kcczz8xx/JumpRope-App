 ...
 * 請使用 `@/features/school-service` 中的 `createCourseAction`、`deleteCourseAction` 和 `getCourses`
 * 此文件保留作為參考，將在未來版本移除
 */
    ...
    try {
        ...
        const course = await prisma.schoolCourse.create({
            data: {
                ...
                chargingModels: chargingModel ? [chargingModel] : [],
                studentPerLessonFee: studentPerLessonFee
                    ? parseFloat(studentPerLessonFee)
                    : null,
                studentFullCourseFee: totalCourseFee
                    ? parseFloat(totalCourseFee)
                    : null,
